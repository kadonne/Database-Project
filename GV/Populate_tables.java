...
public class Populate_tables
{
	...
	//Factory ==============================================================================================
	static String[] f_name = { "Mineral Furnace", "Furnace Fire","Fortune Furnace", "fac4", "fac5", "fac6" };
	static String[] f_address = { "3258 Chapel Street, NY", "2743 Fairmont Avenue, CA", "4365 Wright Court, KY", "2517 Lynn Avenue PA", "166 Progress Way TX", "2799 Huntz Lane LA"};
	...
	static String[] f_manager = { "John", "Dave", "Mike", "Jesus", "Goku", "Satan" };
	//Factory ==============================================================================================
	...
	public static void make_factories()
	{
		...
		for (int i=0;i<6;i++)
		{
			System.out.printf("insert into factory values ('%d', '%s', '%s', '%s', '%s', '%s');%n", 
			numbers.get(i), f_name[i], f_address[i], f_zip_code[i], f_phone[i], f_manager[i]);
			fac_id.add(numbers.get(i));
		}
	}
	...
	public static void make_jobs()
	{
		...
		for (int job=0;job<10;job++)
		{
			System.out.printf("insert into job values ('%d', '%s', '%d', '%d', '%d', '%s','%d','%s','%d','%s');%n", 
				job_code.get(job),j_title[(int)(Math.random()*4)], fac_id.get((int)(Math.random()*5)),
				pos_code.get((int)(Math.random()*10)),
				per_id.get((int)(Math.random()*4)),emp_mode[(int)(Math.random()*2)],
